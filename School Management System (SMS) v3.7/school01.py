...
def INSERT_SC_TEACH_REC():
    ...
    while ans.lower()=='i':
        ...
        t_q=input("Enter the teacher's Qualification(degree) as B.Sc, BCA or B.Tech for the Bachelor's degree and as M.Sc, MCA or M.Tech for the Master's degree (Note:- B.Ed is compulsory): ").upper()
        if '.' in t_q:
            t_q=t_q.replace('.','')
        print()
        ...
        if j==ID:
            ...
            while ans.lower()=='o':
                ...
                if ch=='N':
                    ...
                    print()
                    print("// Note:- Do not give dot in between while typing the teacher's degree //")
                    q=input("Enter the teacher's updated Qualification(degree) as B.Sc, BCA or B.Tech for the Bachelor's degree and as M.Sc, MCA or M.Tech for the Master's degree (Note:- B.Ed is compulsory): ").upper()
                    if '.' in q:
                        q=q.replace('.','')
                    sch[j][2]=q
                    print("*** TEACHER'S DETAIL HAS BEEN SUCCESSFULLY UPDATED !!! ***")
