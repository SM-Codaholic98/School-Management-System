...
def INSERT_ARTS_TEACH_REC():
    f=open('SMS_ARTS_TEACH.dat','wb')
    ans='i'
    while ans.lower()=='i':
        print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        ...
        t_q=input("Enter the teacher's Qualification(degree) as either BA or BFA for the Bachelor's degree and as either MA or MFA for the Master's degree (Note:- B.Ed is compulsory): ").upper()
        if '.' in t_q:
            t_q=t_q.replace('.','')
        print() 
...
bp2=35000   
bp3=40000    
def COMPUTE_ARTS_TEACH_REC():
    
    global percent,bp1,bp2,bp3
    
    f=open('SMS_ARTS_TEACH.dat','rb+')
    sch={}
    while True:
        try:
            sch=pickle.load(f)
     ...
     for i in sch:
       if i==ID:
        ...
        if sch[i][9]>21 and sch[i][9]<61:
            if "BED" in sch[i][2] and ("BA" in sch[i][2] or "BFA" in sch[i][2]):
                if ("MA" not in sch[i][2] and "MFA" not in sch[i][2]) and "PHD" not in sch[i][2]:
                ...
                print("~~~ NOTE: Teacher's ID NUMBER -",i,",NAME -",sch[i][0],",AGE -",sch[i][9],"& QUALIFICATION -",sch[i][2],"will be appointed in the school, if the teacher must have B.Ed and (BA or BFA) ~~~")    
        ...
        if j==ID:
            ...
            while ans.lower()=='o':
                ...
                if ch=='N':
                    ...
                    print()
                    print("// Note:- Do not give dot in between while typing the teacher's degree //")
                    q=input("Enter the teacher's updated Qualification(degree) as either BA or BFA for the Bachelor's degree and as either MA or MFA for the Master's degree (Note:- B.Ed is compulsory): ").upper()
                    if '.' in q:
                        q=q.replace('.','')
                    sch[j][2]=q
                    print("*** TEACHER'S DETAIL HAS BEEN SUCCESSFULLY UPDATED !!! ***")
